// GlitchIt — Heleket crypto-payment proxy (Vercel serverless function).
// The browser can't sign Heleket requests (MD5 over base64(body) + secret API
// key, which must never ship client-side), so the app calls these same-origin
// endpoints and this function does the signing with the API key + merchant id
